...
export default async function PortalPage() {
  ...
  const events = eventsForGuestGroup(guest.group);
  ...
  const t = locale === "es"
    ? {
        ...
        rsvpText:
          "Por favor confirma antes del 1 de junio de 2026. Puedes actualizar tu respuesta cuando quieras.",
      }
    : {
        note: "A Note For You",
        rsvp: "RSVP",
        ...
        rsvpText:
          "Please submit your RSVP by June 1, 2026. You can return and update your response anytime.",
      };
    ...
    <div className="space-y-6">
      <section className="grid gap-4 rounded-3xl border border-stone-200 bg-white/85 p-6 shadow-sm md:grid-cols-[1.2fr_1fr]">
        ...
        <div className="relative h-44 overflow-hidden rounded-2xl border border-stone-200">
          <Image src={TEMP_IMAGES.venue} alt="Guest note image" fill className="object-cover" />
        </div>
      </section>
      ...
      <section className="rounded-3xl border border-stone-200 bg-white/90 p-6 shadow-sm">
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-4">
          {[TEMP_IMAGES.moment1, TEMP_IMAGES.moment2, TEMP_IMAGES.moment3, TEMP_IMAGES.moment4].map(
            (imageSrc, index) => (
              <div
                key={imageSrc}
              ...
              >
                <Image src={imageSrc} alt={`Portal gallery ${index + 1}`} fill className="object-cover" />
              </div>
            ),
          )}
        </div>
      ...
      <section className="grid gap-4 md:grid-cols-2">
        {events.map((event) => (
          ...
          >
            <p className="text-xs uppercase tracking-[0.18em] text-stone-500">
              {dayLabelMap[event.dayLabel as keyof typeof dayLabelMap] ?? event.dayLabel}
            </p>
